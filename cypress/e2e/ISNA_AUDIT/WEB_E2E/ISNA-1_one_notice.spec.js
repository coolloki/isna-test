...
describe('ISNA-1_TDD. Тестирование созданий извещений, с постусловием 1', { tags: ['smoke'] }, () => {
    before(() => {
    ...
    it(`Создание Извещения для ${testDataSource[0].tp_reg.iinBin} с формой проверки "${testDataSource[0].de_audit_form.name.ru}" и вопросом проверки "${testDataSource[0].de_audit_question.name.ru}"`, function () {
        ...
        const thisCaseData = JSON.parse(JSON.stringify(testDataSource[0]).replace(/\:null/gi, "\:\"\""))
        ...
        //«Статус» - Создано,
        noticeJournal.checkNoticeStatus('created')
